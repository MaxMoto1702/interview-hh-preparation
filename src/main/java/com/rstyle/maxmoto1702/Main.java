...
public class Main {
    ...
    public static String executeGet(String targetURL, String query, Map<String, String> headers) {
        ...
        try {
            ...
            connection = new URL(!query.equals("") ? targetURL + "?" + query : targetURL).openConnection();
...
//        LOG.info(executeGet("https://m.hh.ru/oauth/authorize", "response_type=code&client_id="));
        ...
        headers.put("User-Agent", "MyApp/1.0 (my-app-feedback@example.com)");
        headers.put("Host", "api.hh.ru");
        headers.put("Accept", "*/*");
        headers.put("Authorization", tokenType + " " + accessToken);

        String response = executeGet("http://api.hh.ru/me", "", headers);
